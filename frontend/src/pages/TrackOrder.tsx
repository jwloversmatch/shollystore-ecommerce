...
const TrackOrder = () => {
  ...
  const isAuthMode = !!orderId && !!user;

  const tokenQuery = useTrackByTokenQuery(token, { skip: !isTokenMode });
  const authQuery = useTrackMyOrderQuery(orderId, { skip: !isAuthMode });
  ...
  const {
    ...
    isError,
    error,
  } = isTokenMode ? tokenQuery : isAuthMode ? authQuery : { data: undefined, isFetching: false, isError: false, error: undefined };
  ...
  const shouldShowResult = !isFetching && !!order;
          ...
          {!shouldShowResult && !isFetching && (
            ...
            >
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                {isError ? errorMessage : "Please use the tracking link from your order confirmation email."}
              </p>
              <button
