...
interface LocalVariant {
  sku?: string; color?: string; size?: string; price?: number; stock?: number; compareAtPrice?: number;
}
...
const colorMap: Record<string, string> = {
  black:  '#0f0f0f', white:  '#f5f5f5', red:    '#dc2626', blue:   '#2563eb',
  green:  '#16a34a', pink:   '#ec4899', purple: '#9333ea', orange: '#f97316',
  yellow: '#eab308', gray:   '#6b7280', brown:  '#78350f', navy:   '#1e3a8a',
  beige:  '#d4c5a9', gold:   '#f59e0b', silver: '#c0c0c0',
};
...
const ProductDetail = () => {
  ...
  const variantSizes = useMemo(() => {
    ...
    variants.forEach(v => { if (v.size?.trim()) sizes.add(v.size.trim()); });
  ...
  const handleAddToCart = () => {
    ...
    const variantInfo = activeVariant
      ...
      : undefined;
    dispatch(addToCart({
      _id: product._id, name: product.name, image: product.images?.[0] || PLACEHOLDER,
      price: displayPrice, qty, stock: displayStock, variant: variantInfo,
    }));
          ...
          <div className="flex justify-center mb-5">
            <div className="relative">
              ...
              <div className="w-20 h-20 rounded-full flex items-center justify-center"
                ...
                <ImageOff className="w-9 h-9" style={{ color:ACCENT }} aria-hidden="true" />
              </div>
            </div>
          </div>
      ...
      <nav aria-label="Breadcrumb" className="flex items-center gap-2 mb-5 overflow-x-auto no-scrollbar pb-1">
        ...
        {categoryNode && (
          <div className="flex items-center gap-1.5 text-xs shrink-0">
            ...
            <ChevronRight className="w-3.5 h-3.5 text-gray-400 dark:text-gray-600" aria-hidden="true" />
            <Link to={`/shop/${categoryNode.slug}`} className="font-semibold transition-colors" style={{ color:ACCENT }} aria-current="page">
              {categoryNode.name}
            </Link>
          </div>
        )}
      ...
      <div className="grid md:grid-cols-2 gap-6 md:gap-10 lg:gap-14 items-start">
        ...
        <section aria-label="Product images" className="md:sticky md:top-24 space-y-3">
          <div className="relative rounded-2xl md:rounded-3xl overflow-hidden
            ...
            <div className="flex items-center justify-center p-4 md:p-8" style={{ minHeight:200, maxHeight:320 }}>
            ...
            {isOutOfStock && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/60" aria-hidden="true">
                <span className="px-5 py-2 rounded-full font-black text-sm text-white bg-red-500/90">Out of Stock</span>
              </div>
            )}
          </div>
          ...
          {/* Thumbnails */}
          {images.length > 1 && (
            <div className="flex gap-2.5 overflow-x-auto pb-1 no-scrollbar" role="listbox" aria-label="Product image thumbnails">
              {images.map((img, idx) => (
                <motion.button
                  key={idx}
                  role="option"
                  ...
                  <img src={getCloudinaryUrl(img, 100)} alt="" loading="lazy" className="w-full h-full object-cover" />
                </motion.button>
              ))}
            </div>
          )}
        ...
        <section aria-label="Product details" className="space-y-5">
          <div>
            <div className="flex items-center gap-2 mb-2.5">
              <div className="w-5 h-5 rounded-lg flex items-center justify-center shrink-0" style={{ background:`${ACCENT}18` }}>
                <Tag className="w-3 h-3" style={{ color:ACCENT }} aria-hidden="true" />
              </div>
              <span className="text-[10px] font-extrabold uppercase tracking-[0.22em]" style={{ color:ACCENT }}>{categoryName}</span>
            </div>
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {colorsToShow.map(color => {
                      ...
                      return (
                        ...
                        >
                          {!hex && (
                            <span className="text-xs font-black uppercase text-gray-800 dark:text-white">{color.charAt(0)}</span>
                          )}
                          {selectedColor === color && (
          ...
          <div className="flex items-baseline gap-3 flex-wrap" aria-label="Product pricing">
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-3xl sm:text-4xl font-black" style={{ color:ACCENT }}>{displayPrice.toLocaleString()}</span>
            </div>

            {hasVariants && !activeVariant && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-[#e8622a]/10 text-[#e8622a] border border-[#e8622a]/25">Starting Price</span>
            )}
            ...
            {hasSalePrice && displayCompareAtPrice && (
              <span className="text-gray-400 dark:text-gray-500 line-through text-xl font-medium" aria-label={`Original price ₦${displayCompareAtPrice.toLocaleString()}`}>
                ₦{displayCompareAtPrice.toLocaleString()}
              </span>
            )}
            {hasDiscount && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-extrabold bg-red-500/20 text-red-400 border border-red-500/30" aria-label={`${discountPercent}% discount`}>
                -{discountPercent}%
              </span>
            )}
          </div>
          ...
          {product.tags && product.tags.length > 0 && (
            <div className="flex flex-wrap gap-2" aria-label="Product tags">
              {product.tags.map((tag, idx) => (
                <span key={idx} className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-white/5 dark:bg-white/5 text-gray-500 dark:text-gray-400 border border-gray-300 dark:border-white/10">{tag}</span>
              ))}
            </div>
          )}
          ...
          {!isOutOfStock && (
            <div className="hidden sm:flex items-center gap-3">
              <div className="flex items-center rounded-xl overflow-hidden shrink-0 bg-gray-100 dark:bg-[#1c1c1c] border border-gray-200 dark:border-white/[0.09]" aria-label="Quantity selector">
                <button onClick={() => qty > 1 && setQty(q=>q-1)} disabled={qty<=1}
                ...
                <span className="w-10 text-center text-lg font-black text-gray-900 dark:text-white select-none" aria-live="polite">{qty}</span>
                <button onClick={() => qty < displayStock && setQty(q=>q+1)} disabled={qty>=displayStock}
              ...
              <button onClick={handleAddToCart}
                ...
                {added ? <Check className="w-5 h-5" aria-hidden="true" /> : <ShoppingCart className="w-5 h-5" aria-hidden="true" />}
          ...
          <div className="grid grid-cols-2 gap-2.5 pt-3 border-t border-gray-200 dark:border-white/[0.06]" role="list" aria-label="Product details">
            {[
              ...
              <div key={item.label} className="p-3 rounded-xl bg-gray-100 dark:bg-[#1c1c1c] border border-gray-200 dark:border-white/[0.07]" role="listitem">
            ...
            <div className="bg-[#FCFAF5] dark:bg-[#0A0A0B] px-4 pb-3 pt-2 border-t border-gray-200 dark:border-white/[0.07]">
              <div className="flex gap-2.5">
                <div className="flex items-center rounded-xl overflow-hidden shrink-0 bg-white dark:bg-[#141414] border border-gray-200 dark:border-white/[0.1]" aria-label="Quantity selector">
                  ...
                  <span className="w-8 text-center text-base font-black text-gray-900 dark:text-white select-none" aria-live="polite">{qty}</span>
                  <button onClick={() => qty < displayStock && setQty(q=>q+1)} disabled={qty>=displayStock}
                ...
                <button onClick={handleAddToCart}
                  ...
                  style={{ background: added ? '#10b981' : ACCENT, boxShadow: added ? '0 6px 20px rgba(16,185,129,0.35)' : `0 6px 20px ${ACCENT}44` }}
                  aria-label={added ? `${product.name} added to cart` : `Add ${product.name} to cart for ₦${displayPrice.toLocaleString()}`}>
