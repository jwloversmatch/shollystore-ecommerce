...
import {
  ShoppingCart, ArrowLeft, ImageOff, Minus, Plus, Check,
  Tag, Truck, RefreshCw,
} from 'lucide-react';
...
const ProductDetail = () => {
  ...
  // Fetch product data
  const { data: productsData, isLoading: productsLoading } = useGetProductsQuery({ limit: 9999 });
  const products: ProductItem[] = productsData?.products ?? [];
  const product = products.find((p) => p.slug === slug);

  // Fetch category tree for breadcrumbs
  ...
  const breadcrumbs = categoryNode
    ? [
        { name: 'Shop', path: '/shop' },
        { name: categoryNode.name, path: `/shop/${categoryNode.slug}` },
      ]
    : [];
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
      ...
      <div className="flex items-center gap-3 mb-8 flex-wrap">
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-bold text-gray-500 hover:text-white transition-colors"
          ...
          <div className="flex items-center gap-2 text-sm">
            {breadcrumbs.map((crumb, idx) => (
              <span key={idx} className="flex items-center gap-2">
                ...
                <Link to={crumb.path} className="font-bold text-gray-500 hover:text-white transition-colors">
                  {crumb.name}
                </Link>
              </span>
            ))}
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-14 items-start">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1, duration: 0.5 }}
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 rounded-3xl flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.55)' }}>
              <div className="px-6 py-2.5 rounded-full font-black text-sm" style={{ background: 'rgba(239,68,68,0.9)', color: '#fff' }}>
                Out of Stock
              </div>
            </div>
          )}
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15, duration: 0.5 }}
          ...
          {/* Stock status */}
          <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl" style={{ background: '#1c1c1c', border: '1px solid rgba(255,255,255,0.07)' }}>
            <motion.div
