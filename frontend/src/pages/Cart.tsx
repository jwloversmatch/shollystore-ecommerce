    ...
    >
      ...
      >
        ...
        <motion.button
          ...
          aria-label={t("cart.empty.browseAria")}
        >
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        aria-label={`${t("cart.clearAll")} ${totalItems} ${totalItems === 1 ? t("cart.item") : t("cart.items")}`}
    ...
    >
      ...
      <div className="flex-1 min-w-0 w-full">
        ...
        <p className="text-sm font-semibold mt-0.5" style={{ color: ACCENT }}>
          ₦{item.price.toLocaleString()} / {t("cart.unit")}
        </p>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {user ? (
          <div className="mb-5">
            <p className="text-[10px] font-extrabold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-2.5 flex items-center gap-1.5">
              <Tag className="w-3 h-3" aria-hidden="true" /> {t("cart.discountCode")}
            </p>
          ...
          <div className="mb-5 p-4 rounded-xl bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-sm text-blue-800 dark:text-blue-200">
            <p className="flex items-start gap-2">
              ...
              <span>
                {t("cart.loginForCouponPrefix")}{" "}
                <Link to="/login" className="font-bold underline hover:opacity-80 transition-opacity">
                  {t("cart.signIn")}
                </Link>{" "}
                {t("cart.or")}{" "}
                <Link to="/register" className="font-bold underline hover:opacity-80 transition-opacity">
                  {t("cart.createAccount")}
                </Link>{" "}
                {t("cart.loginForCouponSuffix")}
              </span>
            </p>
          </div>
        )}
        ...
        <div className="mt-4 flex items-center justify-center gap-3" aria-label={t("cart.acceptedPayments")}>
          {["💳", "🏦", "💬"].map((icon, i) => (
            <div
              key={i}
          ...
          >
            ...
            >
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-7">
                {t("cart.clearCartMessage", { count: totalItems })}
              </p>
              ...
              <div className="flex gap-3">
                ...
                <motion.button
                  ...
                  aria-label={`${t("cart.clearCartConfirmAria", { count: totalItems })}`}
                >
...
const Cart = () => {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      toast.success(t("cart.couponApplied", { amount: res.coupon.discount.toLocaleString() }));
    } catch (err: unknown) {
      const e = err as { data?: { message?: string } };
      setCouponError(e?.data?.message || t("cart.invalidCoupon"));
    }
  };
    ...
    >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-8">
          <div className="lg:col-span-2 space-y-3" aria-label={t("cart.items")}>
