import { Banknote, Building, MessageCircle, AlertCircle, CheckCircle2 } from "lucide-react";
...
const PaymentDetails = ({ settings, isDark }: PaymentDetailsProps) => {
  ...
  const defaultAccount = settings?.bankAccounts?.find((acc) => acc.isDefault && acc.isActive)
        ...
        {defaultAccount ? (
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: inputBg, border: `1px solid ${inputBorder}` }}>
                <Building className="w-4 h-4 mt-0.5 shrink-0" style={{ color: "#10b981" }} />
                <div>
                  <p className="text-[9px] font-extrabold uppercase tracking-[0.2em] mb-1" style={{ color: textMuted }}>Bank</p>
                  <p className="font-semibold text-sm" style={{ color: textPrimary }}>{defaultAccount.bankName}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: inputBg, border: `1px solid ${inputBorder}` }}>
                <Banknote className="w-4 h-4 mt-0.5 shrink-0" style={{ color: "#10b981" }} />
                <div>
                  <p className="text-[9px] font-extrabold uppercase tracking-[0.2em] mb-1" style={{ color: textMuted }}>Account Name</p>
                  <p className="font-semibold text-sm" style={{ color: textPrimary }}>{defaultAccount.accountName}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: inputBg, border: `1px solid ${inputBorder}` }}>
                <Banknote className="w-4 h-4 mt-0.5 shrink-0" style={{ color: "#10b981" }} />
                <div>
                  <p className="text-[9px] font-extrabold uppercase tracking-[0.2em] mb-1" style={{ color: textMuted }}>Account Number</p>
                  <p className="font-mono font-semibold text-sm tracking-widest" style={{ color: textPrimary }}>{defaultAccount.accountNumber}</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 p-5 rounded-xl" style={{ background: inputBg, border: `1px solid ${inputBorder}`, color: textMuted }}>
            ...
            <p className="text-sm">No bank accounts configured yet. Please add one in the admin settings.</p>
          </div>
        )}

        {/* WhatsApp number (still flat) */}
        {settings?.whatsappNumber && (
          <div className="mt-4 flex items-start gap-3 p-4 rounded-xl" style={{ background: inputBg, border: `1px solid ${inputBorder}` }}>
            <MessageCircle className="w-4 h-4 mt-0.5 shrink-0" style={{ color: "#25D366" }} />
            <div>
              <p className="text-[9px] font-extrabold uppercase tracking-[0.2em] mb-1" style={{ color: textMuted }}>WhatsApp</p>
              <p className="font-semibold text-sm" style={{ color: textPrimary }}>{settings.whatsappNumber}</p>
            </div>
          </div>
        )}
      </div>
