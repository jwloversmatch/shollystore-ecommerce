...
const ProductsPage = () => {
  ...
  const filteredProducts = useMemo(() => {
    let f = products;
    if (categoryFilter !== "All") f = f.filter((p) => getCategoryId(p.category) === categoryFilter);
  ...
  if (isLoading) {
    return (
      <main id="main-content" tabIndex={-1} className="p-4 md:p-6 pt-16 md:pt-24 max-w-7xl mx-auto space-y-5 pb-28 md:pb-10 focus:outline-none" style={{ background: bg }}>
        <div className="rounded-2xl overflow-hidden" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
          {Array.from({ length: 8 }).map((_, i) => <ProductRowSkeleton key={i} dark={isDark} />)}
        </div>
      </main>
    );
  }
  ...
  return (
    <main id="main-content" tabIndex={-1} className="p-4 md:p-6 pt-16 md:pt-24 max-w-7xl mx-auto space-y-5 pb-28 md:pb-10 focus:outline-none" style={{ background: bg }}>
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-black leading-none" style={{ color: textPrimary }}>Products</h1>
            <p className="text-xs mt-0.5" style={{ color: textMuted }} aria-live="polite">{products.length} products in catalog</p>
          </div>
        </div>
        <button
      ...
      {/* Filter bar */}
      <div className="flex flex-col sm:flex-row gap-3 rounded-2xl p-4" style={{ background: cardBg, border: `1px solid ${cardBorder}` }} role="search" aria-label="Filter products">
        <div className="relative flex-1">
          <label htmlFor="product-search" className="sr-only">Search products</label>
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: textMuted }} aria-hidden="true" />
          <input
            id="product-search"
            type="search"
