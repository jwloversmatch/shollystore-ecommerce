  ...
  return (
    <main id="main-content" tabIndex={-1} className="min-h-screen p-4 md:p-6 pt-16 md:pt-24 max-w-4xl mx-auto pb-28 md:pb-10 space-y-5 focus:outline-none" style={{ background: "#0A0A0B" }}>
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600 text-xs mt-0.5" aria-live="polite">{categories.length} categories · Organise your products</p>
          </div>
        </div>

        <button onClick={() => { setIsCreating(true); setEditingId(null); }} className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-white text-sm shrink-0" style={{ background: ACCENT, boxShadow: `0 6px 18px ${ACCENT}44` }} aria-label="Add new category">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.28 }} className="overflow-hidden" role="region" aria-label="Create category form">
            <div className="relative rounded-2xl p-4 md:p-5" style={{ background: "#141414", border: `1px solid ${ACCENT}40`, boxShadow: `0 0 0 1px ${ACCENT}20, 0 8px 32px rgba(0,0,0,0.4)` }}>
              ...
              <div className="space-y-3">
                ...
                <input id="new-category-name" type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Category name (e.g., Soft Drinks)" className={inputCls} autoFocus onKeyDown={(e) => { if (e.key === "Enter") handleCreate(); if (e.key === "Escape") { setIsCreating(false); setNewName(""); } }} />

                <label htmlFor="new-category-parent" className="sr-only">Parent category</label>
                <select id="new-category-parent" value={newParent} onChange={(e) => setNewParent(e.target.value)} className={`${inputCls} cursor-pointer`} aria-label="Select parent category">
              ...
              <div className="flex gap-2 mt-4">
                <button onClick={handleCreate} className="flex items-center gap-1.5 px-4 py-3 rounded-xl font-bold text-white text-sm" style={{ background: ACCENT }}>
                  <Check className="w-4 h-4" aria-hidden="true" /> Save
                </button>
                <button onClick={() => { setIsCreating(false); setNewName(""); setNewParent(""); }} className="flex items-center gap-1.5 px-4 py-3 rounded-xl font-bold text-gray-500 hover:text-white transition-colors text-sm" style={{ background: "#1c1c1c", border: "1px solid rgba(255,255,255,0.08)" }}>
      ...
      <div className="space-y-3" role="list" aria-label="Category list">
        {!categories.length ? (
          <div className="relative rounded-2xl p-10 text-center" style={{ background: "#141414", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="flex justify-center mb-5">
              <div className="relative">
                <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background: `${ACCENT}12`, boxShadow: `0 0 0 3px ${ACCENT}` }}>
                  <FolderOpen className="w-9 h-9" style={{ color: ACCENT }} aria-hidden="true" />
                </div>
              </div>
            </div>
            <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] mb-2" style={{ color: ACCENT }}>Empty</p>
            <h2 className="text-xl font-black text-white mb-2">No categories yet</h2>
            <p className="text-gray-600 text-sm mb-6">Create your first category to start organising products.</p>
            <button onClick={() => setIsCreating(true)} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white text-sm" style={{ background: ACCENT, boxShadow: `0 6px 18px ${ACCENT}44` }}>
              <Plus className="w-4 h-4" aria-hidden="true" /> Create First Category
            </button>
          </div>
        ) : (
                  ...
                  {editingId === cat._id ? (
                    <div className="p-4 md:p-5" role="group" aria-label={`Editing category: ${cat.name}`}>
                      ...
                      <div className="space-y-3">
                        ...
                        <input id={`edit-category-name-${cat._id}`} type="text" value={editingName} onChange={(e) => setEditingName(e.target.value)} className={inputCls} autoFocus onKeyDown={(e) => { if (e.key === "Enter") handleUpdate(cat._id); if (e.key === "Escape") { setEditingId(null); setEditingName(""); } }} />
                        <label htmlFor={`edit-category-parent-${cat._id}`} className="sr-only">Parent category</label>
                        <select id={`edit-category-parent-${cat._id}`} value={editingParent} onChange={(e) => setEditingParent(e.target.value)} className={`${inputCls} cursor-pointer`} aria-label="Select parent category">
                          <option value="">Top‑level (no parent)</option>
                          {categories.filter((c: Category) => c._id !== cat._id).map((c: Category) => (
                            <option key={c._id} value={c._id}>{c.name}</option>
                          ))}
                        </select>
                      </div>
                      <div className="flex gap-2 mt-4">
                        <button onClick={() => handleUpdate(cat._id)} className="flex items-center gap-1.5 px-4 py-3 rounded-xl font-bold text-white text-sm" style={{ background: ACCENT }} aria-label={`Save changes to ${cat.name}`}>
                          <Check className="w-4 h-4" aria-hidden="true" /> Save
                        </button>
                        <button onClick={() => { setEditingId(null); setEditingName(""); setEditingParent(""); }} className="flex items-center gap-1.5 px-4 py-3 rounded-xl font-bold text-gray-500 hover:text-white transition-colors text-sm" style={{ background: "#1c1c1c", border: "1px solid rgba(255,255,255,0.08)" }}>
                          <X className="w-4 h-4" aria-hidden="true" /> Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 md:p-5 group">
                      <div className="flex items-center gap-3.5 flex-1 min-w-0">
                        <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 text-lg font-black" style={{ background: `${ACCENT}15`, color: ACCENT, border: `1px solid ${ACCENT}25` }} aria-hidden="true">
                          <Tag className="w-4 h-4" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <h3 className="font-black text-white text-base truncate">{cat.name}</h3>
                          <div className="flex items-center gap-2.5 mt-1 flex-wrap">
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.06)", color: "#6b7280" }}>/{cat.slug}</span>
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1" style={{ background: cat.parent ? "rgba(59,130,246,0.1)" : "rgba(16,185,129,0.1)", color: cat.parent ? "#60a5fa" : "#34d399" }}>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 ml-4">
                        <button onClick={() => { setEditingId(cat._id); setEditingName(cat.name); setEditingParent(cat.parent?._id || ""); setIsCreating(false); }} className="w-9 h-9 rounded-xl flex items-center justify-center transition-colors" style={{ background: "rgba(59,130,246,0.1)", color: "#60a5fa", border: "1px solid rgba(59,130,246,0.2)" }} aria-label={`Edit category ${cat.name}`}>
                          <Edit className="w-4 h-4" aria-hidden="true" />
                        </button>
                        <button onClick={() => { setToDelete(cat._id); setDeleteModal(true); }} className="w-9 h-9 rounded-xl flex items-center justify-center transition-colors" style={{ background: "rgba(239,68,68,0.08)", color: "#f87171", border: "1px solid rgba(239,68,68,0.18)" }} aria-label={`Delete category ${cat.name}`}>
