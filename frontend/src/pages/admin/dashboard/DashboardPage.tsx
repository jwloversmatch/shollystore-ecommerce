...
const DashboardPage = () => {
  ...
  const { data: revenueTrendData } = useGetRevenueTrendQuery(trendDays);
  ...
  if (statsLoading || productsLoading || analyticsLoading) {
    return (
      <main
        id="main-content"
        tabIndex={-1}
        className="min-h-screen p-4 md:p-6 max-w-7xl mx-auto space-y-5 md:space-y-6 pb-28 md:pb-10 focus:outline-none pt-[calc(56px_+_env(safe-area-inset-top,0px))] md:pt-[calc(80px_+_env(safe-area-inset-top,0px))] lg:pt-[calc(88px_+_env(safe-area-inset-top,0px))]"
        style={{ background: bg }}
      >
  ...
  return (
    ...
    >
      ...
      >
        {[
          ...
          },
          {
            ...
            path: "/admin/products?lowStock=true",
          },
        ].map((s, i) => (
          <button
            key={i}
      ...
      <RecentReviewsWidget isDark={isDark} />
