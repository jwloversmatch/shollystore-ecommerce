  ...
  verifyWebhookSignature(payload: string, signature: string): boolean {
    ...
    try {
      ...
      return (
        ...
        timingSafeEqual(hashBuffer, sigBuffer)
      );
    } catch {
      return false;
    }
  ...
  private validateInitializeInput(
    ...
    if (!this.config.clientUrl) {
      throw new ValidationError(
        "clientUrl is required for payment initialization",
      );
    }
  }
  ...
  private handleError(error: unknown): PaystackError {
    if (axios.isAxiosError(error)) {
      ...
      if (axiosErr.response) {
        ...
        const paystackMsg =
          axiosErr.response.data?.message ?? "Unknown Paystack error";
        return new PaystackError(
          `Paystack request failed (${status})`,
          status,
          paystackMsg,
        );
      }
