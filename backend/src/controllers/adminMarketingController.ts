...
export const sendMarketingEmail = async (
  ...
  try {
    ...
    if (!type || !productId) {
      res
        .status(400)
        .json({ success: false, message: "Type and productId are required." });
      return;
    }
    ...
    if (!["new_arrival", "back_in_stock"].includes(type)) {
      res
        ...
        .json({
          success: false,
          message: "Invalid type. Must be new_arrival or back_in_stock.",
        });
      return;
    }
    ...
    if (recipients.length === 0) {
      res
        .status(400)
        .json({ success: false, message: "No customers to send to." });
      return;
    }
    ...
    res.json({
      success: true,
      message: `Emails are being sent to ${recipients.length} customers.`,
    });
  } catch (error: any) {
